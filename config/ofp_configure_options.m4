...
AC_ARG_ENABLE(c-actions, 
    AC_HELP_STRING([--enable-c-actions],
                   [enable OFP C actions (default: enabled)]))
# the hyphen in c-actions is converted to an underscore.
if test "$enable_c_actions" = "no"; then
    AC_MSG_RESULT([no])
    OFP_ENABLE_C_ACTIONS="no"
else
    AC_MSG_RESULT([yes])
    enable_c_actions="yes"
...
AC_ARG_ENABLE(c-main, 
    AC_HELP_STRING([--enable-c-main],
                   [enable OFP C main (default: enabled)]))
# the hyphen in c-actions is converted to an underscore.
if test "$enable_c_main" = "no"; then
    AC_MSG_RESULT([no])
